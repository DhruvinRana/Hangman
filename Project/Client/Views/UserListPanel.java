...
import Project.Client.Interfaces.IUserListEvent;
 ...
 */
public class UserListPanel extends JPanel implements IReadyEvent, IPointsEvent, ITurnEvent, IUserListEvent {
    ...
    @Override
    public void onUserAwayStatus(long clientId, boolean isAway) {
        if (userItemsMap.containsKey(clientId)) {
            SwingUtilities.invokeLater(() -> {
    ...
    public void onUserSpectatorStatus(long clientId, boolean isSpectator) {
        if (userItemsMap.containsKey(clientId)) {
            SwingUtilities.invokeLater(() -> {
                userItemsMap.get(clientId).setSpectator(isSpectator);
            });
        }
    }

    public void addUser(User user) {
        UserListItem item = new UserListItem(user.getClientId(), user.getDisplayName(), this);
        item.setAway(user.isAway());
        item.setSpectator(user.isSpectator());
