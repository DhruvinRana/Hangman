...
public abstract class BaseGameRoom extends Room {
    ...
    protected void handleReady(ServerThread sender) {
        try {
            ...
            // NEW: If all players are ready, start the session immediately
            long numReady = clientsInRoom.values().stream().filter(p -> p.isReady()).count();
            if (numReady >= MINIMUM_REQUIRED_TO_START && numReady == clientsInRoom.size()) {
                resetReadyTimer();
                onSessionStart();
            } else {
                startReadyTimer(false); // fallback: start timer if not all ready
            }
        } catch (Exception e) {
    ...
    public void broadcastPayload(Project.Common.Payload payload) {
